...
description: >-
  可选能力：仅维护使用者已独立配置的 VitePress 文档站，包括已确认原型或文档的同步、构建、部署、鉴权、版本和站点故障排查。
  PMCockpit 不提供 docs-site、服务器或凭证；未显式安装或未配置站点时不触发。远端写入前须先确认目标并 dry-run。
---

# VitePress 文档站部署与维护

本 Skill 是 PMCockpit 的可选能力，仅在使用者已执行 `bash .ai/install.sh --with-vitepress`，并拥有独立文档站、部署权限和授权范围时使用。框架不携带 `docs-site/`、服务器、账号、密码或密钥；没有站点时只交付本地原型和待同步清单，不执行部署。

它可维护采用「VitePress 静态站点 + Express 鉴权服务 + PM2 守护 + rsync 部署」架构的既有文档站。纯 VitePress、GitHub Pages 或其他架构应按实际能力裁剪，不把本示例当成强制依赖。
...
> 原型 HTML 位于 `public/`，但线上 Express 服务的是 `.vitepress/dist/`；替换后必须重新 build，才能把新文件复制到 `dist/`。

## 部署命令模板

以下是完整的「改了源码 → 上线」流程。每次远端操作前确认部署用户、服务器、密钥引用、远端目录、目标站点和改动范围；不得把这些信息写入工具记忆或 Git。

```bash
KEY="<密钥路径>"
HOST="<服务器地址>"
DEPLOY_USER="<部署用户>"
DOCS="<本地 docs-site 路径>"
REMOTE_DOCS_DIR="<远端 docs-site 路径>"
...
rsync -azn --itemize-changes \
  ...
  "$DOCS/" "$DEPLOY_USER@$HOST:$REMOTE_DOCS_DIR/"

# 3. 预览无误后执行正式同步；仅在用户确认清理远端多余文件时追加 --delete
rsync -az \
  -e "ssh -i $KEY -o BatchMode=yes -o IdentitiesOnly=yes -o PubkeyAcceptedAlgorithms=+ssh-rsa" \
  --exclude='node_modules/' --exclude='.git/' --exclude='.vitepress/dist/' \
  --exclude='.vitepress/cache/' --exclude='.DS_Store' \
...
ssh -i "$KEY" -o BatchMode=yes -o IdentitiesOnly=yes \
    -o PubkeyAcceptedAlgorithms=+ssh-rsa "$DEPLOY_USER@$HOST" \
    "source ~/.nvm/nvm.sh; cd '$REMOTE_DOCS_DIR'; npm run build; pm2 restart <pm2进程名>"
```
...
- `../需求文档/XX平台——MVP需求文档V1.md`
- → `/versions/v1.0.0/requirements/XX平台——MVP需求文档V1.0.0`（去掉 `.md` 后缀，注意文件名要对上）
...
```bash
ssh -i <密钥> -o IdentitiesOnly=yes -o PubkeyAcceptedAlgorithms=+ssh-rsa <部署用户>@<服务器地址>
```
...
```bash
ssh ... <部署用户>@<服务器地址> 'source ~/.nvm/nvm.sh; cd <远端站点路径>; npm run build; pm2 restart <pm2进程名>'
```
...
**症状**：上传 V1.0.1 原型后，V1.0.0 的同名原型变成了 V1.0.1 的内容。

**根因**：不同版本的原型 HTML 文件名相同（如 V1.0.0 和 V1.0.1 都有 `E0-企业概览-原型.html`），放在同一目录会互相覆盖。
...
**根因**：源项目目录（如 `02当前项目/<项目名>/.../04-原型/`）和 docs-site 目录是两个独立位置。rsync 只推 docs-site，源项目文件不在推送范围。
...
1. SSH 到服务器
2. source ~/.nvm/nvm.sh
3. 编辑 `<远端站点路径>/server/auth.js` 中的认证配置
4. `pm2 restart <pm2进程名>`
```
...
每个项目独立一套站点，各自端口、各自密码、各自目录。解决同站多项目的内容泄露问题（坑 7）。
...
每站复制现有站结构，改 4 样：`config.mts` 的 title、`auth.js` 的密码、`ecosystem.config.cjs` 的 PORT+name（PM2 进程名也要不同）、文档和原型内容。

> ⚠️ 内存限制：每站 ~65MB，2G 内存服务器建议最多 4 个站。超过需升级内存或改 Nginx 统一代理方案。
...
> `config.mts` 的 `base` 路径：GitHub Pages 部署用 `/repo-name/`，自部署用 `/`（可用环境变量 `VITEPRESS_BASE` 控制）。
> VitePress 目录首页必须叫 `index.md`，不识别 `_index.md`（那是 Docusaurus 约定）。

## 当前部署实例与凭证

服务器地址、SSH 密钥、站点密码等敏感凭证只能存放在使用者受控的本地环境，不写入 Skill、工具记忆或 Git。PMCockpit 不规定其具体落点。

| 项 | 值 |
|----|-----|
| 项目 | XX平台 |
| Node | v22.22.0（nvm） |
| 内容隔离 | 单一密码保护，站点只含单项目内容。多项目需分站点部署（物理隔离） |
