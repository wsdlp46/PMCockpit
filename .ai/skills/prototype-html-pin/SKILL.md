...
description: >
  生成交互式HTML产品原型。采用 scaffold 源文件 + build 构建的工作流，公共样式/侧边栏/Pin 标注脚本集中维护。
...
2. **风格判定** - 按页面类型定视觉方案：管理后台/B端 → 用 `scaffold/` 体系（见“技术栈要求”）；门户/C端 → 读 `01通用规则/设计库/web门户-design.md`；移动端H5 → 读 `01通用规则/设计库/手机端-design.md`；数据大屏 → 深色底+卡片矩阵。同时读 `01通用规则/设计库/设计约定.md`。未明确端态默认按管理后台处理。
...
- 门户/C端：读 `01通用规则/设计库/web门户-design.md`，单文件 HTML + Tailwind，不走 scaffold
- 移动端H5：读 `01通用规则/设计库/手机端-design.md`，单文件 HTML
