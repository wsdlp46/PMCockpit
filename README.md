...
- 正式项目只写在自己的 `02当前项目/<项目名>/`；不要在公开仓库克隆目录或 `examples/` 中积累业务内容。
- 做需求、规格或原型前，先让 AI 读取项目总览、迭代索引和相关关键决策。
- 新会议纪要、客户材料或原始文件，先明确项目和版本，再使用“材料入库”沉淀时间线与待确认决策。
- 修改原型后，先确认原型；再让 AI 列出需要同步的规格和需求文件，确认后才回写。
- 版本或项目阶段结束后，使用“复盘”沉淀问题、改进项与可复用经验。
- 遇到安装或路由问题，先检查 `.ai/TOOL-MAPPING.md`、对应 Skill 的 `SKILL.md` 和演练 README；仍无法解决时，记录工具版本、提示词、报错全文和复现步骤。
