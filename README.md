...
仓库内置了一个不含真实业务数据的模拟项目背景：[examples/模拟项目-智慧园区巡检/](examples/模拟项目-智慧园区巡检/)。它提供从“新建项目 → 启动版本 → PRD → 页面规格 → 原型 → 变更回写”的逐步提示词。

首次使用时，打开样例 README，按顺序把提示词输入 AI；所有产物都在 `02当前项目/` 新建的练习项目中生成，不要直接修改样例目录。
